...
void fnActuadores(String cad,float tempo,unsigned int luxo){
  ...
  if(label.equals("cale")){
    ...
    if(label.equals("c_1")){
      value_c_1=value.toFloat();
     if(value_c_1>  tempo){
      digitalWrite(calefaccion_1,0);
      digitalWrite(calefaccion_2,0);
     }
     else if(value_c_1< tempo){
      digitalWrite(calefaccion_1,1);
      digitalWrite(calefaccion_2,1);
    }
    }
   if(label.equals("c_2")){
      value_c_2=value.toInt();
      
      digitalWrite(foco,value_c_2);
